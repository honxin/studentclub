...
<body >
	...
	@endif
	   	...
	   	<div class="ui form segment" style="margin:2%" >
    		<table class="ui basic table">
			  <tbody>
			  	...
			  	<tr>
			    	<td>社团LOGO</td>
			    	<td>
			    		<span id="atten"></span><span id="filename"></span>
			    		<form  id="addfile" action="{{ action('Admin\AssocInfoController@upload') }}" method="post" enctype="multipart/form-data" target='uploadToThis'>
			    ...
			    <tr>
			    	...
			    	<td>
			    		<div class="field">
						    <textarea name="description" id="description" placeholder="社团简介"></textarea>
						</div>
			    	</td>
			    </tr>
		    ...
		    <input class="ui blue submit button" type="button" id="addAssocInfo" value="添加" style="background-color:#19B2EB" />
	   	</div>
	   	<script>
	   		$('#addAssocInfo').click(function(){
				...
				$.ajax({
					type : 'post',
					url  : '{{ action('Admin\AssocInfoController@store') }}',
					data : {_method:method,_token:token,assoc_id:assoc_id,logo:filename,description:description,detailcontent:content},
